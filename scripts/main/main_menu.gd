...
func _build_layered_screen() -> void:
	...
	_build_language_picker(layer_stage)
	...
	_place_layer(goblin_layer, 0.17, 0.27, 0.83, 0.70)
	...
	texture_button.tooltip_text = LanguageManager.t("start.adventure")
	...
	var button_copy: VBoxContainer = UITheme.make_tr_dual("start.adventure", 42, 17, UITheme.INK)
	...
	_build_pvp_entry(layer_stage)

func _build_pvp_entry(parent: Control) -> void:
	# Compact entry to the PvP lobby, squeezed between the goblin art and the
	# adventure button.
	var pvp_button: Button = Button.new()
	pvp_button.name = "PvpEntryButton"
	pvp_button.focus_mode = Control.FOCUS_NONE
	pvp_button.mouse_default_cursor_shape = Control.CURSOR_POINTING_HAND
	pvp_button.tooltip_text = LanguageManager.t("pvp.entry")
	var fill: Color = UITheme.MINT
	for state: String in ["normal", "hover", "pressed", "disabled"]:
		pvp_button.add_theme_stylebox_override(state, UITheme.rounded_style(fill, Color("#28654f"), 999, 4))
	pvp_button.add_theme_stylebox_override("hover", UITheme.rounded_style(fill.lightened(0.06), Color("#28654f"), 999, 4))
	pvp_button.add_theme_stylebox_override("pressed", UITheme.rounded_style(fill.darkened(0.05), Color("#28654f"), 999, 4))
	var button_copy: VBoxContainer = UITheme.make_tr_dual("pvp.entry", 40, 16, UITheme.MINT_DARK)
	button_copy.name = "PvpEntryButtonText"
	button_copy.set_anchors_and_offsets_preset(Control.PRESET_FULL_RECT)
	button_copy.offset_left = 40.0
	button_copy.offset_right = -40.0
	button_copy.mouse_filter = Control.MOUSE_FILTER_IGNORE
	pvp_button.add_child(button_copy)
	_place_layer(pvp_button, 0.325, 0.705, 0.675, 0.755)
	pvp_button.pressed.connect(_on_pvp_pressed)
	parent.add_child(pvp_button)

func _on_pvp_pressed() -> void:
	if _transitioning:
		return
	AudioManager.play_sfx("button_click")
	get_tree().change_scene_to_file.call_deferred("res://scenes/pvp/pvp_lobby.tscn")

func _build_language_picker(parent: Control) -> void:
	# Three compact buttons; each language name renders in its own font so the
	# TC face never has to cover kana, and the JP face never has to cover the
	# rare Traditional glyphs it lacks.
	var row: HBoxContainer = HBoxContainer.new()
	row.name = "LanguagePickerRow"
	...
	for language: String in LanguageManager.LANGUAGES:
		...
		button.name = "LanguageButton_%s" % language
		button.custom_minimum_size = Vector2(148, 80)
		button.size_flags_vertical = Control.SIZE_SHRINK_CENTER
		button.focus_mode = Control.FOCUS_NONE
		button.mouse_default_cursor_shape = Control.CURSOR_POINTING_HAND
		button.text = LanguageManager.language_name(language)
		button.add_theme_font_override("font", UITheme.shared_font_for_language(language, UITheme.FontRole.BOLD))
		...
		var selected: bool = language == LanguageManager.get_language()
		var fill: Color = Color("#ffe19a") if selected else Color(1.0, 0.97, 0.92, 0.92)
		var border: Color = Color("#d7a45d") if selected else Color("#e4c9a6")
		button.add_theme_stylebox_override("normal", UITheme.rounded_style(fill, border, 24, 3))
		button.add_theme_stylebox_override("hover", UITheme.rounded_style(fill.lightened(0.06), border.lightened(0.06), 24, 3))
		button.add_theme_stylebox_override("pressed", UITheme.rounded_style(fill.darkened(0.05), border.darkened(0.08), 24, 3))
		button.disabled = selected
		button.pressed.connect(_on_language_selected.bind(language))
		row.add_child(button)

func _on_language_selected(language: String) -> void:
	AudioManager.play_sfx("button_click")
	LanguageManager.set_language(language)
	# All copy is built in _ready, so a reload is the clean path for every scene.
...
func _build_fallback_screen() -> void:
	...
	var title: Label = UITheme.make_tr_label("start.title", 62, UITheme.INK)
	...
	var fallback_button: Button = UITheme.make_tr_button("start.adventure", UITheme.YELLOW, Vector2(0, 150))
